...
fn solve_and_apply(
    ...
    for (target, summary, reflective) in partial {
        ...
        measurements.push(CalibrationMeasurement {
            client_id: target.client_id.clone(),
            name: target.name.clone(),
            measured_delay_ms: Some(summary.delay_seconds * 1000.0),
            deviation_ms: Some(summary.deviation_seconds * 1000.0),
            accepted: summary.accepted as u32,
            rejected: summary.rejected as u32,
            intrinsic_latency_ms: Some(solution.intrinsic_latency_seconds * 1000.0),
